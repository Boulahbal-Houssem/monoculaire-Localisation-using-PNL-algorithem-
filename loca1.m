...
% Stereo rig
matPos_c1_c2=[0.9999 -0.0076 0.0047 -95.3538;0.0075 0.9999 -0.0034 -2.2704;-0.0046 0.0035 0.9999 -0.5507;0 0 0 1];
matPos_c2_c1=inv(matPos_c1_c2);
...
for p=1:N
    ...
    % ********************************************************************
    % Partie 1.3-1: Calcul de normale
    %   ******* A completer ******* %
    l1 = [(u1(p)-u0_1) / alphaU_1; (v1(p)-v0_1)/ alphaV_1 ; 1];
    l2 = [(u2(p)-u1(p)) / alphaU_1 ; (v2(p)-v1(p))/ alphaV_1 ; 0];
    
    normale = cross(l1,l2) / abs( norm( cross(l1,l2) ) )
...
oldCrit=EvalCrit(N,App2d,App3d_mod);                 % Criteria evaluation
...
while (1),
    ...
    newCrit=EvalCrit(N,App2d,App3d_mod);
